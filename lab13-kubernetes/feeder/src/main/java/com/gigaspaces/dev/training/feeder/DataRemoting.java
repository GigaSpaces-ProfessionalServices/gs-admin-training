...
/**
 * A bean that uses OpenSpaces Remoting support to invoke IDataProcessor implementation exposed by
 ...
 *
 * <p>Note, this bean simply uses IDataProcessor, OpenSpaces Remoting hides the fact that this
 * interface will actually cause a remote invocation, with the Space as the transport layer,
 * directed into a service exposed by another processing unit.
 *
 * @author kimchy
 */
public class DataRemoting {

    private static final Logger logger = Logger.getLogger(DataRemoting.class.getName());

    private long numberOfTypes = 10;

    private long defaultDelay = 1000;

    @ExecutorProxy(gigaSpace = "gigaSpace", timeout = 30000)
    ...
    public void construct() {
        logger.log(Level.INFO, "--- STARTING REMOTING WITH CYCLE [" + defaultDelay + "]");
    ...
    public class DataFeederTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                logger.log(Level.INFO, "--- REMOTING PARAMETER " + data);
                dataProcessor.sayData(data);
                data = dataProcessor.processData(data);
                logger.log(Level.INFO, "--- REMOTING RESULT   " + data);
